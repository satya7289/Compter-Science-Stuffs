    ...
    while not stack.isEmpty(): 
        ...
        if poped_state[0] not in visited:
            ...
            if problem.isGoalState(poped_state[0]):
                return BacktrackUtil(problem,poped_state,Dic)
...
def BacktrackUtil(problem, poped_state, Dic):
    directions = []
    while True:
        if problem.getStartState() in poped_state:
            break
        directions.append(poped_state[1])
        # print type(poped_state), poped_state
        poped_state = tuple(Dic[poped_state][0])
    # print directions
    return directions[::-1]
